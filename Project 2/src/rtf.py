...
def estimate_rtf_prewhiten(mic_signals, path_signals, ref_mic_idx=0, fs=16000, nperseg=N_PER_SEG, noverlap=N_OVERLAP, alpha=0.99, det_threshold=0.01, pre_cheat_time=50):
    ...
    for l in range(num_time_samples):
        for k in range(num_wave_numbers):
            ...
            if l >= pre_cheat_time:
                ...
                # # Decide on the detection threshold to update the noise space
                # corr_score = np.abs(np.dot(max_eigenvectors, max_eigenvectors_prev) / (np.linalg.norm(max_eigenvectors) * np.linalg.norm(max_eigenvectors_prev)))  
